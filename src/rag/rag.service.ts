...
export class RagService {
  ...
  async query(userQuery: string, filters?: QueryFilters): Promise<RagResponse> {
    const queryVector = await this.embeddingService.embed(userQuery);

    const chunks = await this.vectorRepo.findSimilar(queryVector, TOP_K, SIMILARITY_THRESHOLD);
    ...
    if (chunks.length === 0) {
      return {
        answer: 'No relevant job postings found for your query. Try different keywords or broaden your search.',
        sources: [],
